...
pub mod gamemodes{
    ...
    pub fn set_gamemode(){
        ...
        println!("[smashline_krool::data] HDR: {}",hdr_enabled);
        ...
        println!("[smashline_krool::data] Ult-S: {}",ultS_enabled);
        {
            if hdr_enabled{
...
const IDENTIFIER: &str = "smashline_krool";
...
fn copy_dir_all(src: impl AsRef<Path>, dst: impl AsRef<Path>) -> io::Result<()> {
    ...
    for entry in std::fs::read_dir(src)? {
        ...
        if ty.is_dir() {
            ...
            println!("[smashline_krool::data] copied {} to {}",entry.file_name().to_str().unwrap(),dst.as_ref().to_str().unwrap());
        }
    }
    Ok(())
}
...
{
    unsafe {
        let motionFolder = format!("{}/fighter/krool/motion/kroolbarrel",&*MOD_DIR.read().unwrap());
        let slots=8;
        if !Path::new(motionFolder.as_str()).exists()
        {
            println!("[smashline_krool::data] WTF?");
            return;
        }
        ...
        for slot in 1..slots {
            ...
            println!("[smashline_krool::data] copied motion files to {}",destFolder);
            /*
            fs::create_dir_all(destFolder.as_str());
            let destFile = format!("{}/{}",destFolder,file);
            fs::copy(sourceFile.as_str(), destFile.as_str());
            println!("[smashline_krool::data] copied motion files to {}",destFile); */
        }
